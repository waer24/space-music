<template>
  <scroll :scroll-data="listData" :probe-type="probeType" :listen-scroll="listenScroll"
  class="list-view" ref="listview">
    <ul>
      <li class="list-stripe" v-for="(group,index) in listData" :key="index">
        ...
        <ul>
          <li class="text-item" v-for="(item, index) in group.items" :key="index" ref="listItem">
    ...
    <div class="sidebar" @touchstart.stop.prevent="onShortcutTouchStart"  @touchmove.stop.prevent="onShortcutTouchMove" @touchend.stop>
      <ul>
        <li v-for="(item,index) in shortcutList" :data-index ="index" class="side-item" :class="{'active': currentIndex === index}">{{ item }}</li>
      </ul>
    </div>
